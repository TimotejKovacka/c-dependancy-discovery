...
/*
* general data structure used:
* A list implementation of worker queue
* mutex variable for thread safe access and mutation of list
* conditional variable for communication between threads
*/
...
public:
    /*
    * A safe way to insert an element to the back of work queue
    * and notify another threads that work has been addded
    */
    ...
    /*
    * A safe aproach to find out the size of work queue
    * we lock here since size() is used to determine if mutation should take place elsewhere
    */
    unsigned long size() {
        std::unique_lock<std::mutex> lock(mutex_);
        return workQ.size();
    ...
    /*
    * Wait on a promise that work queue has something in it or 10 milliseconds have passed
    * if 10 milliseconds have passed return nothing otherwise pop element from the from
    * and return popped element
    */
    std::string wait_and_pop() {
        std::unique_lock<std::mutex> lock(mutex_);
        auto time_now = std::chrono::system_clock::now();
        cv.wait_until(lock, time_now + std::chrono::milliseconds(10), [this] { return workQ.size() > 0; });
        if (workQ.size() == 0) { return ""; };
...
private:
    ...
    std::mutex mutex_;
public:
    // Safely insert key, value pair into HashMap
    void insert(std::string key, std::list<std::string> value) {
        std::unique_lock<std::mutex> lock(mutex_);
        hashMap.insert(std::pair<std::string, std::list<std::string>>(key, value));
    }

    // Safely push value to the back of list associated with key
    void push_back(std::string key, std::string value) {
        std::unique_lock<std::mutex> lock(mutex_);
        hashMap[key].push_back(value);
    }

    // Safely look up if a key is present in hashMap
    bool is_present(std::string key) {
        std::unique_lock<std::mutex> lock(mutex_);
        return hashMap.find(key) != hashMap.end();
    }

    //Look up if key is in HashMap if so return a pointer to the value it's paired with otherwise return nullptr
    std::list<std::string>* find_value_pointer(std::string key) {
        std::unique_lock<std::mutex> lock(mutex_);
        auto value = hashMap.find(key);
        return value != hashMap.end() ? &hashMap[key] : nullptr;
    }
};
...
static void process(const char* file) {
    ...
    while (fgets(buf, sizeof(buf), fd) != NULL) {
        ...
        *q = '\0';
        ...
        theTable.push_back(file, name);
        // 2bii. if file name not already in table ...
        if (theTable.is_present(name)) { continue; }
...
void thread_process() {
    while (workQ.size() > 0) {
        std::string filename = workQ.wait_and_pop();
        if (filename == "") { continue; }
        if (!theTable.is_present(filename.c_str())) {
            fprintf(stderr, "Mismatch between table and workQ, file: %s\n", filename.c_str());
            exit(-1);
        }
...
static void printDependencies(std::unordered_set<std::string>* printed,
    ...
    while (toProcess->size() > 0) {
        ...
        std::list<std::string>* ll = theTable.find_value_pointer(name);
...
int main(int argc, char* argv[]) {
    ...
    int CRAWLER_THREADS = getenv("CRAWLER_THREADS") == NULL ? 1 : atoi(getenv("CRAWLER_THREADS"));
    ...
    // Create as many threads as specified from enviromental variable
    std::vector<std::thread> threads;
    for (int i = 0; i <= CRAWLER_THREADS; i++)
    {
        auto thread = std::thread([=] {thread_process(); });
